...
function actualizarSgPreview() {
  ...
  function aplicarConfigEEPP(conEEPP) {
    ...
    var nSG  = document.getElementById('step-n-sg');
    var nLO  = document.getElementById('step-n-lo');
    var nFT  = document.getElementById('step-n-ft');
    ...
    btnOk.onclick=function(){
      ...
      if(layoutGlobal.img){
        ...
        currentMarkers.forEach(function(m,i){
          ...
          tctx.fillStyle=hr2(m.color,0.3);
          tctx.strokeStyle=m.color; tctx.lineWidth=1.5;
          ...
          var r=iconR;
          var lw=Math.max(2.2,Math.round(r*0.088)); // +10% grosor
          tctx.save();
          tctx.translate(nx,ny);
  ...
  function _generarPPT() {
    ...
    var secciones=['1. Datos del proyecto','2. Estatus de documentación','3. Estatus de aprobación de proyectos'];
    secciones.push('4. Control Curva S');
    if(_conEEPP){ secciones.push('5. Control de Estados de Pago'); secciones.push('6. Situación general de la obra'); }
    else secciones.push('5. Situación general de la obra');
    var base=_conEEPP?6:5;
    if(pendArrastre.length>0) secciones.push((base+1)+'. Puntos de arrastre — semana anterior');
    var layNum=pendArrastre.length>0?base+2:base+1, fotNum=pendArrastre.length>0?base+3:base+2, anxNum=pendArrastre.length>0?base+4:base+3;
    ...
    // Posiciones fijas según especificación
    var leyX=3.252, progY=1.9449, realY=2.2756, desvY=2.6024, leyH=0.36;
    var tblX=0.1181, tblY=2.0157;
    s6.addText(lblProg,{x:leyX,y:progY,w:6.5,h:leyH,fontSize:14,fontFace:FONT,bold:true,color:'888888',valign:'middle'});
    s6.addText(lblReal,{x:leyX,y:realY,w:6.5,h:leyH,fontSize:14,fontFace:FONT,bold:true,color:'1a5fa8',valign:'middle'});
    s6.addText(lblDesv,{x:leyX,y:desvY,w:6.5,h:leyH,fontSize:14,fontFace:FONT,bold:true,color:desvC, valign:'middle'});
    // Gráfico a la derecha de la tabla
    var gfxX=3.252, gfxW=6.4173; var gfxY=desvY+leyH+0.12;
    ...
    if(csRows.length>0){
      ...
      // LIMITE exacto: 17.75cm = 6.9882"
      // pptxgenjs agrega ~2pt de padding vertical por celda (top+bottom)
      // Factor de corrección: descontar el padding antes de dividir
      ...
      // Fuente adaptativa según rowH disponible
      var tblFontHead, tblFontBody;
      if(rowHFinal >= 0.20)      { tblFontHead=12; tblFontBody=10; }
      else if(rowHFinal >= 0.17) { tblFontHead=10; tblFontBody=9;  }
      else if(rowHFinal >= 0.14) { tblFontHead=9;  tblFontBody=8;  }
      else if(rowHFinal >= 0.11) { tblFontHead=8;  tblFontBody=7;  }
    ...
    // — Tabla contrato (izquierda, ancho ~3.5") —
    var cRows=[
      [{text:'CONTRATO',options:{colspan:2,bold:true,fill:'1a3a5c',color:'FFFFFF',align:'center'}}],
      [{text:'Costo Directo',options:{bold:true}}, pptFmt(epCD2)],
      ['GG ('+Math.round(epP.pGG*10000)/100+'%)', pptFmt(epGG2)],
      ['UTI ('+Math.round(epP.pUTI*10000)/100+'%)', pptFmt(epUTI2)],
      [{text:'Subtotal',options:{bold:true}},{text:pptFmt(epSUB2),options:{bold:true}}],
      [{text:'Desc.',options:{color:'C0392B'}},{text:pptFmt(epDESC2),options:{color:'C0392B'}}],
    ...
    var eppY0 = CONT_Y_EP;
    var cRowsH=[
      // Fila de etiquetas — últimas 2 columnas (% Anticipo, % Retenciones) en font 7
      [{text:'Costo Directo',options:{bold:true,fill:'2C4770',color:'FFFFFF',fontSize:10,align:'center'}},
       ...
       {text:'% Anticipo',options:{fill:'2C4770',color:'FFFFFF',fontSize:7,align:'center'}},
       {text:'% Retenciones',options:{fill:'2C4770',color:'FFFFFF',fontSize:7,align:'center'}}],
      // Fila de valores — últimas 2 en font 7, resto en font 10
      [{text:pptFmt(epCD2),options:{bold:true,fontSize:10,align:'center'}},
       {text:pptFmt(epGG2),options:{fontSize:10,align:'center',color:'555555'}},
       {text:pptFmt(epUTI2),options:{fontSize:10,align:'center',color:'555555'}},
       {text:pptFmt(epSUB2),options:{bold:true,fontSize:10,align:'center'}},
       {text:epDESC2>0?pptFmt(epDESC2):'—',options:{fontSize:10,align:'center',color:'C0392B'}},
       {text:pptFmt(epNETO2),options:{bold:true,fontSize:10,align:'center'}},
       {text:pptFmt(epIVA2),options:{fontSize:10,align:'center'}},
       {text:pptFmt(epTOT2),options:{bold:true,fontSize:10,align:'center',color:'1A3A5C'}},
       {text:pptPct(epP.pANT*100),options:{fontSize:7,align:'center'}},
       {text:pptPct(epP.pRET*100),options:{fontSize:7,align:'center'}}]
    ];
    ...
    var eppY = EEPP_Y_EP;  // posición exacta 9.53cm
    {
      ...
      epFilas.forEach(function(f,i){
        ...
        tCD2+=r.cdEP;tGG2+=r.gg;tUTI2+=r.uti;tS12+=r.sub1;tDC2+=r.desc;
        tS22+=r.sub2;tAT2+=r.ant;tRT2+=r.ret;tNT2+=r.neto;tIV2+=r.iva;tTT2+=r.tot;
        ...
        eppRows2.push([
          ...
          c(pptFmt(r.sub1),{align:'right'}), c(pptFmt(r.desc),{align:'right',color:'C0392B'}),
      ...
      eppRows2.push([
        ...
        ct(pptFmt(tDC2),{align:'right'}),ct(pptFmt(tS22),{align:'right'}),
    ...
    function newSgSlide(){
      slideNum++;
      var sl=prs.addSlide();
      addHF(sl,(_conEEPP?'6':'5')+'. Situación general de la obra',slideNum);
    ...
    function newSgArrastreSlide(){
      slideNum++;
      var sl=prs.addSlide();
      addHF(sl,(_conEEPP?'7':'6')+'. Puntos de arrastre — semana anterior',slideNum);
      // Línea divisoria vertical entre columnas
      sl.addShape(prs.ShapeType.rect,{
        x:COL_L+COL_W+0.03, y:CONT_Y+0.02,
        w:0.01, h:MAX_Y-CONT_Y-0.05,
  ...
  function buildPreviewSlides(){
    ...
    var secciones2=['1. Datos del proyecto','2. Estatus de documentación','3. Estatus de aprobación de proyectos',
      '4. Control Curva S'];
    if(_conEEPP){ secciones2.push('5. Control de Estados de Pago'); secciones2.push('6. Situación general de la obra'); secciones2.push('7. Lay Out Arquitectura vigente'); secciones2.push('8. Fotografías relevantes'); if(anexos.length>0) secciones2.push('9. Anexos'); }
    else { secciones2.push('5. Situación general de la obra'); secciones2.push('6. Lay Out Arquitectura vigente'); secciones2.push('7. Fotografías relevantes'); if(anexos.length>0) secciones2.push('8. Anexos'); }
